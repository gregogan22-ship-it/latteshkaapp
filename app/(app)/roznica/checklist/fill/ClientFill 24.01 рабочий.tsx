...
export default function ClientFill() {
  ...
  // === ЛОКАЛЬНЫЙ КЭШ ===
  const CACHE_KEY = `checklist_${cafe || "no-cafe"}_${gasDate || "no-date"}_${role || "no-role"}_${category || "no-category"}`;
  ...
  useEffect(() => {
    async function loadPerformed() {
      if (!cafe || !gasDate || !role || !category) return setPerformed([]);
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      ...
      {loadingPerformed && <div className="text-center py-4 text-gray-500">Загрузка...</div>}
