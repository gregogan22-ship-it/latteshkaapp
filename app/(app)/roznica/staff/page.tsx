...
// Вставь свои данные Supabase (Settings → API)
...
export default function SkladSchedule() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const errMsg = "Ошибка загрузки данных: " + err.message;
        setError(errMsg);
        toast.error(errMsg);
        console.error(err);
      } finally {
  ...
  const handleChange = async (employeeId: string, day: string, value: string) => {
    try {
      const { error } = await supabase
        .from("sklad_schedule")
        .upsert(
          { employee_id: employeeId, day, value },
