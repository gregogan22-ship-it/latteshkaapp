...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log('[LOGIN] Пытаемся войти под:', trimmedNickname);

      // Ищем пользователя по nickname
      const { data: profile, error } = await supabase
        .from('profiles')
        .select('user_id, full_name, role, coffee_shop, password')
        .eq('nickname', trimmedNickname)
        .single();

      if (error || !profile) {
        console.log('[Login] Ошибка или пользователь не найден:', error);
        throw new Error('Пользователь не найден');
      }

      console.log('[Login] Найден профиль:', profile);

      // Проверяем пароль из таблицы profiles
