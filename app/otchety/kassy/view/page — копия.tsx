...
// ← ВСТАВЬ ЭТУ ФУНКЦИЮ ЗДЕСЬ
  ...
  // Расчёты для дополнительного блока
  ...
  // Средняя касса текущего месяца (с 11 по 10 следующего)
  const monthStart = periodStart + "T00:00:00";
  const monthEnd = periodEnd + "T23:59:59.999";
  ...
  // Средняя касса прошлого года (тот же период год назад)
  const lastYearMonthStart = getDateNDaysAgo(periodStart, 365);
  const lastYearMonthEnd = getDateNDaysAgo(periodEnd, 365);
          ...
          >
              ...
              <div className="flex justify-between items-center">
                ...
                <div className="flex gap-4">
                  <Link href="/otchety/kassy/add">
                    <button className="bg-white text-blue-700 px-6 py-4 rounded-xl font-bold text-lg hover:bg-gray-100 transition">
                      + Внести кассу
                    </button>
                  </Link>
                  <Link href="/otchety/kassy/edit">
                    <button className="bg-orange-600 text-white px-6 py-4 rounded-xl font-bold text-lg hover:bg-orange-700 transition">
            ...
            <div className="p-6 bg-gray-50 flex flex-wrap gap-4">
              ...
              <select
                ...
                className="px-6 py-3 rounded-xl border bg-white outline-none font-medium"
              >
            ...
            <div className="p-8 space-y-12">
                  ...
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="bg-indigo-100">
                        <th className="px-6 py-3 text-center">Время</th>
                        <th className="px-6 py-3 text-center">Оборот прошлой недели</th>
                        <th className="px-6 py-3 text-center">Касса прошлой недели</th>
                        <th className="px-6 py-3 text-center">Оборот прошлого дня</th>
                        <th className="px-6 py-3 text-center font-bold text-xl">Оборот текущего дня</th>
                        <th className="px-6 py-3 text-center font-bold text-xl">Касса текущего дня</th>
                        <th className="px-6 py-3 text-center">Количество чеков</th>
                        <th className="px-6 py-3 text-center">Средний чек</th>
                      </tr>
                    </thead>
                    <tbody>
                      {TIME_SLOTS.map((time) => {
                        ...
                        return (
                          <tr key={time} className="border-b hover:bg-indigo-50">
                            <td className="px-6 py-4 text-center font-medium">{time}</td>
                            <td className="px-6 py-4 text-center text-gray-600">{lastWeekTurnover.toLocaleString()} ₽</td>
                            <td className="px-6 py-4 text-center text-gray-600">{lastWeekCash.toLocaleString()} ₽</td>
                            <td className="px-6 py-4 text-center text-gray-600">{yesterdayTurnover.toLocaleString()} ₽</td>
                            <td className={`px-6 py-4 text-center font-bold text-xl ${turnoverBg}`}>{currentTurnover.toLocaleString()} ₽</td>
                            <td className={`px-6 py-4 text-center font-bold text-xl ${cashBg}`}>{currentCash.toLocaleString()} ₽</td>
                            <td className="px-6 py-4 text-center">{currentChecks}</td>
                            <td className="px-6 py-4 text-center">{avgCheck.toLocaleString()} ₽</td>
                          </tr>
                        );
                      })}
                    </tbody>
              ...
              {displayedCafes.map((cafeName) => {
                ...
                return (
                  <div key={cafeName} className="bg-white rounded-2xl shadow-lg p-8">
                        ...
                        {TIME_SLOTS.map((time) => {
                          ...
                          return (
                            <tr key={time} className="border-b hover:bg-gray-50">
                              <td className="px-6 py-4 text-center font-medium">{time}</td>
                              <td className="px-6 py-4 text-center text-gray-600">{lastWeekSlot.turnover.toLocaleString()} ₽</td>
                              <td className="px-6 py-4 text-center text-gray-600">{lastWeekSlot.cash.toLocaleString()} ₽</td>
                              <td className="px-6 py-4 text-center text-gray-600">{yesterdaySlot.turnover.toLocaleString()} ₽</td>
                              <td className={`px-6 py-4 text-center font-bold text-xl ${turnoverBg}`}>{currentSlot.turnover.toLocaleString()} ₽</td>
                              <td className={`px-6 py-4 text-center font-bold text-xl ${cashBg}`}>{currentSlot.cash.toLocaleString()} ₽</td>
                              <td className="px-6 py-4 text-center">{currentSlot.checks}</td>
                              <td className="px-6 py-4 text-center">{avgCheck.toLocaleString()} ₽</td>
                            </tr>
                          );
                        })}
